...
public class UserController {
    ...
    public ResponseEntity<UserDTO> registerUser(@Valid @RequestBody UserDTO userDTO) {
        ...
        String token = this.jwtUtil
                .createToken(userDTO.getLogin(), userDTO.getRoles()
                        .stream()
                        .map(r -> new SimpleGrantedAuthority(r.getName()))
                        .collect(Collectors.toSet()));

        HttpHeaders responseHeaders = new HttpHeaders();
        responseHeaders.add(ACCESS_CONTROL_EXPOSE_HEADERS, AUTHORIZATION);
        responseHeaders.add(AUTHORIZATION, BEARER + token);
        ResponseEntity<UserDTO> response = ResponseEntity.ok().headers(responseHeaders).body(createdUser);
